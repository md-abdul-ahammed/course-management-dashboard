...
import { fetchAdminSingleCourse } from "../../redux/slices/adminCourseSlice";

const AdminEditCourse = () => {
  const dispatch = useDispatch();
  const { adminCourseId } = useParams();
  const {fromLocation} = useParams();

  console.log(typeof fromLocation)
  ...
  const { adminSingleCourse, loading } = useSelector(
    (state) => state.adminCourses
  );

  useEffect(() => {
    dispatch(fetchAdminSingleCourse(adminCourseId));
  }, [dispatch]);
      ...
      <div style={{ background: " #FAFAFB" }}>
        <div className="header">
          ...
          >
            ...
            <Link
            to={fromLocation === "1" ? "/adminInstituteDetails/2/instituteCourse" : "/adminDashboard/requests/courseRequests"}
