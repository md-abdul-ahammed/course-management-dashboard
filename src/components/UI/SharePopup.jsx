...
export default function SharePopup({
  ...
  const hostURL = 'http://ostello.co.in'
  ...
  return (
    <div>
      <Modal open={open} onClose={onClose}>
        <div className=' flex flex-col space-y-5  p-5  sm:w-[400px] max-w-[400px]  rounded-lg text-light-black '>
          <div className='flex justify-between text-2xl items-center '>
            <span className=''>{isReferral ? 'Referral' : 'Share'}</span>
            <CloseOutlined
              onClick={onClose}
              className='flex text-sm  text-desc-gray cursor-pointer rounded-full ring-1 p-1 ring-desc-gray hover:bg-violet hover:text-white '
            />
          </div>
          ...
          <div className=' flex justify-center w-full relative'>
            ...
            <input
              type='text '
              value={TextToCopy || `${hostURL}${pathname}`}
